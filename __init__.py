...
# 初始化系统
game_system = GameSystem(data_dir)
entertainment_system = EntertainmentSystem(config["api_keys"])
tools_system = ToolsSystem(data_dir)
...
async def handle_gamble(bot, event, args: Message = CommandArg()):
    try:
        ...
        success, win_amount = game_system.gamble(user_id, amount, 0.5)
...
async def handle_password(bot, event, args: Message = CommandArg()):
    try:
        ...
        if length < 8 or length > 32:
            await password.finish("密码长度必须在8-32之间！")
            
        pwd = tools_system.generate_password(length)
